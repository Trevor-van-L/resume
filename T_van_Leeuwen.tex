...
\usepackage[left=0.6in, top=0.65in, right=0.6in, bottom=0.65in]{geometry}
\usepackage{multirow}
...
\begin{minipage}[t]{0.66\textwidth} %This will break if we overflow vertically. Need to use a page break if you want two pages
...
\jobtitle{Qualcomm}{Engineer, Linux Audio Software}{C, C++, Python}{Apr 2019 -- July 2023}
\smallskip
\\
...
This central component (GSL) abstracts the DSP interface to provide use-case level control APIs, and is the entry point to the new framework
\smallskip
\begin{itemize}
    ...
    \item {Implemented data transfer mode using run-time application-layer-allocated memory with no memcpy for Android Codec 2.0. Optimized this mode to meet power \& latency KPIs of modes which use setup-time driver-allocated memory}
\comment{    \item {Designed features needing interaction between many other components, ensuring features will work for novel device setups and adaptations}}
...
\begin{itemize}
    \item {Created debugging tool to log binary command packets and make them human-readable, with automatic error detection. Elimintated significant dev time spent looking through binary packet dumps, and improved ability of external customer teams to extend AudioReach framework}
    \item {Trained members from device teams on GSL component to improve their ability to develop new features, accelerating time from design to completion. Refined workflows to move framework towards full open source}
...
\begin{tabularx}{\linewidth}{l| >{\raggedright}X}
    ...
    \textsc{Familiar} & Python, MATLAB, C\# \cr
    \textsc{Tools} & Git, P4, GDB, Valgrind, Bash, Linux, Jenkins \cr
    \textsc{Concepts} & DSP, Embedded, Operating Systems, Kernel Drivers, Latency, Memory \& Power Optimization
\end{tabularx}
\end{adjustwidth}
